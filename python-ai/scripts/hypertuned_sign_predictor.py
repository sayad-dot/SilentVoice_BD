...
class HypertunedSignLanguagePredictor:
    def __init__(self,
                 model_path='../trained_models/hypertuned_bangla_lstm_final.h5',
                 encoder_path='../trained_models/hypertuned_label_encoder.pkl',
                 config_path='../trained_models/hypertuned_model_config.json'):
    ...
    def load_model(self):
        try:
            if os.path.exists(self.model_path):
                ...
                print(f"✅ Loaded hypertuned model from {self.model_path}", file=sys.stderr)
            else:
            ...
            if os.path.exists(self.encoder_path):
                ...
                print(f"✅ Loaded hypertuned label encoder", file=sys.stderr)
            else:
            ...
            if os.path.exists(self.config_path):
                ...
                print(f"✅ Loaded hypertuned model config", file=sys.stderr)
            else:
                self.config = {
                    'sequence_length': 30,
                    'feature_dim': 288,
            ...
            print(f"❌ Error loading hypertuned model: {e}", file=sys.stderr)
            raise
    ...
    def predict_from_pose_sequence(self, pose_sequence):
        try:
            ...
            print(f"✅ Hypertuned prediction: {predicted_text} (confidence: {confidence:.4f})", file=sys.stderr)
            
            return {
                'success': True,
                'predicted_text': predicted_text,
                'confidence': confidence,
                'model_version': 'hypertuned_bangla_lstm_v1'
            }
            
        except Exception as e:
            print(f"❌ Hypertuned prediction error: {e}", file=sys.stderr)
            return {
                'success': False,
                'error': str(e),
                'predicted_text': 'ত্রুটি',
...
def main():
    if len(sys.argv) < 2:
        print(json.dumps({"error": "Usage: python hypertuned_sign_predictor.py '<pose_sequence>'"}))
        return
    
    pose_sequence = json.loads(sys.argv[1])
    predictor = HypertunedSignLanguagePredictor()
